...
func TestBinName(t *testing.T) {
	got := binName()
	if runtime.GOOS == "windows" && got != "vibeperks.cmd" {
...
func TestQuoteForHostShell(t *testing.T) {
	got := quoteForHostShell(`C:\Program Files\vibeperks.cmd`)
	if runtime.GOOS == "windows" {
		if got != `"C:\Program Files\vibeperks.cmd"` {
			t.Errorf("windows quote = %q", got)
		}
	} else if got != `'C:\Program Files\vibeperks.cmd'` {
		t.Errorf("posix quote = %q", got)
	}
}
...
func TestDispatchLoginStoresToken(t *testing.T) {
	...
	// Keep the login's token-verify call offline and fast: an unreachable API
	// makes Verify fail immediately; the token must still be saved regardless.
	t.Setenv("VIBEPERKS_API", "http://127.0.0.1:1")
	old := os.Args

...
```
Sonnet - context 30% - limit 10%   Make your AI pay for itself - VibePerks.ai
```
...
Installing needs **no Go and no manual build**: `bin/vibeperks` (and `bin/vibeperks.cmd`
on Windows) is a committed launcher that downloads the prebuilt binary for your platform
from the GitHub Release on first use and caches it. Building from source is only for
development (Go 1.23+):
...
Set `VIBEPERKS_RELEASE_CHANNEL=dev-latest` to pull the dev prerelease instead of the default
`latest` release.
